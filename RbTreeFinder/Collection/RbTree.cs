...
namespace RbTreeFinder.Collection
{
    ...
    /// </remarks>
    public class RbTree<TKey, TValue>
    {
        ...
        public RbTree(Comparison<TKey> keyComparison)
        {
        ...
        public RbTree(Comparison<TKey> keyComparison, Comparison<TValue> valueComparison)
            : this(keyComparison)
        {
        ...
        public bool Remove(TKey key, TValue value)
        {
            ...
            if (null != RootNode)
            {
                RootNode = Remove(RootNode, key, value);
                if (null != RootNode)
                {
                    RootNode.IsBlack = true;
                }
        ...
        public TValue GetValueForKeyOrGreater(TKey key)
        {
            if (IsMultiDictionary)
            {
                throw new InvalidOperationException("GetValueForKey is only supported when acting as a normal (non-multi) dictionary.");
            }
            Node node = GetNodeForKeyOrGreater(key);
            if (null != node)
            {
                return node.Value;
            }
        ...
        private Node GetNodeForKeyOrGreater(TKey key)
        {
            // Initialize
            Node node = RootNode;
            Node parent = null;
        ...
        {
            // Root is black
            Debug.Assert((null == RootNode) || RootNode.IsBlack, "Root is not black");
            // Every path contains the same number of black nodes
            Dictionary<Node, Node> parents = new Dictionary<RbTree<TKey, TValue>.Node, RbTree<TKey, TValue>.Node>();
            foreach (Node node in Traverse(RootNode, n => true, n => n))
            {
                if (null != node.Left)
                {
            ...
            int treeCount = -1;
            foreach (Node node in Traverse(RootNode, n => (null == n.Left) || (null == n.Right), n => n))
            {
                int pathCount = 0;
                Node current = node;
                while (null != current)
